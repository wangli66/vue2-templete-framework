...
app.use(BASE_URL, function (req, res) {
    ...
    const isStaticHashCache = isStatic && /^\/assets\//.test(req.path);
    if (isStatic) {
    ...
    res.sendfile(join(projectDir, sendfilePath), err => err && res.status(err.status).send(err.status));
});
